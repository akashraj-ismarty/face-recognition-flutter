...
class _HomePageState extends State<RecognitionScreen> {
    ...
    if (await directory.exists()) {
      ...
      if (image) {
        List<FileSystemEntity> imageList =
            files.where((e) => _isImageFile(e.path)).toList();
        ...
        showStringListBottomSheet(context, data, dataNot, image);
      } else {
        showStringListBottomSheet(context, files, [], image);
      }
    } else {
  ...
  Widget _buildBottomSheetContent(BuildContext context,
      List<FileSystemEntity> items, List<FileSystemEntity> second, bool image) {
    return Container(
  ...
  void showStringListBottomSheet(BuildContext context,
      List<FileSystemEntity> items, List<FileSystemEntity> second, bool image) {
    showModalBottomSheet(
      context: context,
      builder: (context) {
        return _buildBottomSheetContent(context, items, second, image);
      },
    );
  }
